...
function(ipGetPrefixPath pfx pth)
  ...
  set(oneValueArgs PKG BASE BRANCH DESC DIST_DIR LICENSE REPO TAG UPSTREAM VER WEB SHA256_Linux SHA256_Linux-arm64 SHA256_win64 URL_Linux URL_Linux-arm64 URL_win64 SHA256_utres)
  ...
  elseif(DEFINED P_REPO AND DEFINED P_TAG AND DEFINED P_SHA256_${SYS_NAME})
    ...
    if(PKG_NAME STREQUAL "exdlpro")
      set(exdlpro_REV ${P_TAG})
      find_package(exdlpro)
      if(DEFINED exdlpro_DIR)
        set(XP_MODULE_PATH ${XP_MODULE_PATH} PARENT_SCOPE)
      else()
        xpGetCompilerPrefix(CMP)
        xpGetNumBits(BITS)
        set(sig ${P_TAG}-${CMP}-${BITS})
        set(url https://${P_REPO}/releases/download/${P_TAG}/${PKG_NAME}-${sig}-${SYS_NAME}-devel.tar.xz)
        set(sig ${sig} PARENT_SCOPE)
        message(STATUS "Finding ${PKG_NAME}-${sig}-${SYS_NAME}...")
      endif()
    else()
...
function(xpFindPkg)
  ...
  foreach(pkg ${FP_PKGS})
    ...
    if(pkg STREQUAL "exdlpro" AND DEFINED sig)
      unset(${pkg}_DIR CACHE)
      find_path(${pkg}_DIR NAMES ${pkg}_${sig}.txt PATHS ${pth}/../../ NO_DEFAULT_PATH)
      mark_as_advanced(${pkg}_DIR)
      list(APPEND XP_MODULE_PATH ${${pkg}_DIR}/share/cmake)
      set(XP_MODULE_PATH ${XP_MODULE_PATH} PARENT_SCOPE)
    elseif(pkg STREQUAL "exdlpro")
      set(XP_MODULE_PATH ${XP_MODULE_PATH} PARENT_SCOPE)
    else()
