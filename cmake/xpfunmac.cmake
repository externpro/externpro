...
function(xpGetSysName sysName)
  if(CMAKE_SYSTEM_PROCESSOR MATCHES "^(aarch64|arm64|ARM64)$")
    set(SYS_NAME "${CMAKE_SYSTEM_NAME}-arm64")
  elseif(CPack_CMake_INCLUDED EQUAL 1)
  ...
  set(${sysName} ${SYS_NAME} PARENT_SCOPE)
endfunction()
...
function(ipDownload url sha dst)
  ...
  file(DOWNLOAD "${url}" "${dst}" EXPECTED_HASH SHA256=${sha} TLS_VERIFY ON STATUS st LOG log)
  list(GET st 0 code)
  list(GET st 1 msg)
  if(NOT code EQUAL 0)
...
function(ipDownloadExtract url sha pth)
  ...
  set(dst ${xprodir}/xpd/pkgs/${txz})
  ipDownload(${url} ${sha} ${dst})
  if(${dst} IS_NEWER_THAN ${xprodir}/xpx)
    file(ARCHIVE_EXTRACT INPUT ${dst} DESTINATION ${xprodir}/xpx)
    file(TOUCH_NOCREATE ${xprodir}/xpx)
  endif()
  string(REPLACE "-xpro.tar.xz" "" pkgbase ${txz})
  string(REPLACE "-devel.tar.xz" "" pkgbase ${pkgbase}) # for old packages
  set(_pth ${xprodir}/xpx/${pkgbase}-xpro/share/cmake)
  if(NOT IS_DIRECTORY "${_pth}")
    set(_pth ${xprodir}/xpx/${pkgbase}/share/cmake)
  endif()
  if(NOT IS_DIRECTORY "${_pth}")
...
function(ipGetPkgSha shafile pkg sha)
  ...
  elseif(CMAKE_SYSTEM_PROCESSOR MATCHES "^(x86_64|amd64|AMD64)$")
    ...
    message(FATAL_ERROR "ipGetPkgSha: unhandled CMAKE_SYSTEM_PROCESSOR='${CMAKE_SYSTEM_PROCESSOR}'")
  endif()
  file(READ "${shafile}" shasums)
  ...
  if(NOT _m)
    message(FATAL_ERROR "ipGetPkgSha: no SHA256SUMS entry matched '${pfx}-${platform}' (or fallback '${platform}') in '${shafile}'")
  endif()
  string(LENGTH "${CMAKE_MATCH_1}" sha_len)
  if(NOT sha_len EQUAL 64)
    message(FATAL_ERROR "ipGetPkgSha: invalid SHA length (${sha_len}) for '${CMAKE_MATCH_2}' in '${shafile}'")
  endif()
  set(${pkg} "${CMAKE_MATCH_2}" PARENT_SCOPE)
  set(${sha} "${CMAKE_MATCH_1}" PARENT_SCOPE)
endfunction()

function(ipGetPrefixPath pfx pth)
  xpGetSysName(SYS_NAME)
  ...
  set(xprodir ${CMAKE_BINARY_DIR}/_xpro)
  # NOTE - xprodir subdirs: xpd (downloaded), xpx (extracted)
  ...
  elseif(DEFINED P_REPO AND DEFINED P_TAG AND DEFINED P_SHA256_${SYS_NAME})
    ...
    set(sha ${P_SHA256_${SYS_NAME}})
  elseif(DEFINED P_SHA_FILE AND EXISTS ${P_SHA_FILE})
    ipGetPkgSha(${P_SHA_FILE} pkg sha) # set pkg and sha
    # assume SHA_FILE and pkg are in the same directory
    cmake_path(GET P_SHA_FILE PARENT_PATH pkg_dir)
    set(url file://${pkg_dir}/${pkg})
  elseif(DEFINED P_PKG AND DEFINED P_REPO AND DEFINED P_TAG AND DEFINED P_SHA256SUMS)
    set(dst ${xprodir}/xpd/shasums/${P_PKG}-${P_TAG}-sha256sums)
    ipDownload(https://${P_REPO}/releases/download/${P_TAG}/SHA256SUMS ${P_SHA256SUMS} ${dst})
    ipGetPkgSha(${dst} pkg sha) # set pkg and sha
    set(url https://${P_REPO}/releases/download/${P_TAG}/${pkg})
  elseif(DEFINED P_PKG)
    message(FATAL_ERROR "ipGetPrefixPath: error in xp_${P_PKG} for ${SYS_NAME}")
  else()
    message(FATAL_ERROR "ipGetPrefixPath: unexpected error")
  endif()
