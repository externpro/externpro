...
function(ipExternPackageExtractLinkItemNames _out items)
  set(_names)
...
function(ipExternPackageInferDeps _out_deps _out_pvt_deps)
  ...
  endforeach()
  ipExternPackageExtractLinkItemNames(_iface_names "${_all_iface}")
  ipExternPackageExtractLinkItemNames(_link_names "${_all_link}")
  set(_pvt_names "${_link_names}")
  if(_pvt_names AND _iface_names)
    list(REMOVE_ITEM _pvt_names ${_iface_names})
  endif()
  if(_iface_names)
    list(REMOVE_DUPLICATES _iface_names)
  endif()
  if(_pvt_names)
    list(REMOVE_DUPLICATES _pvt_names)
  endif()
  set(${_out_deps} "${_iface_names}" PARENT_SCOPE)
  set(${_out_pvt_deps} "${_pvt_names}" PARENT_SCOPE)
endfunction()

function(ipExternPackageAuditDeps)
  cmake_parse_arguments(P "" "REPO_NAME" "TARGETS;DEPS;PVT_DEPS" ${ARGN})
  if(NOT DEFINED P_REPO_NAME OR "${P_REPO_NAME}" STREQUAL "")
    set(P_REPO_NAME "${CMAKE_PROJECT_NAME}")
  endif()
  ipExternPackageInferDeps(_infer_deps _infer_pvt_deps TARGETS ${P_TARGETS})
  set(_provided_deps "${P_DEPS}")
  set(_provided_pvt_deps "${P_PVT_DEPS}")
  set(_missing_deps "${_infer_deps}")
  if(_missing_deps AND _provided_deps)
    list(REMOVE_ITEM _missing_deps ${_provided_deps})
  endif()
  set(_extra_deps "${_provided_deps}")
  if(_extra_deps AND _infer_deps)
    list(REMOVE_ITEM _extra_deps ${_infer_deps})
  endif()
  set(_missing_pvt "${_infer_pvt_deps}")
  if(_missing_pvt AND _provided_pvt_deps)
    list(REMOVE_ITEM _missing_pvt ${_provided_pvt_deps})
  endif()
  set(_extra_pvt "${_provided_pvt_deps}")
  if(_extra_pvt AND _infer_pvt_deps)
    list(REMOVE_ITEM _extra_pvt ${_infer_pvt_deps})
  endif()
  if(_infer_deps OR _infer_pvt_deps OR _provided_deps OR _provided_pvt_deps)
    message(STATUS "xpExternPackage[${P_REPO_NAME}] inferred DEPS='${_infer_deps}' PVT_DEPS='${_infer_pvt_deps}'")
    message(STATUS "xpExternPackage[${P_REPO_NAME}] provided DEPS='${_provided_deps}' PVT_DEPS='${_provided_pvt_deps}'")
    if(_missing_deps OR _extra_deps OR _missing_pvt OR _extra_pvt)
      if(_missing_deps)
        message(STATUS "xpExternPackage[${P_REPO_NAME}] missing DEPS: '${_missing_deps}'")
      endif()
      if(_extra_deps)
        message(STATUS "xpExternPackage[${P_REPO_NAME}] extra DEPS: '${_extra_deps}'")
      endif()
      if(_missing_pvt)
        message(STATUS "xpExternPackage[${P_REPO_NAME}] missing PVT_DEPS: '${_missing_pvt}'")
      endif()
      if(_extra_pvt)
        message(STATUS "xpExternPackage[${P_REPO_NAME}] extra PVT_DEPS: '${_extra_pvt}'")
      endif()
    endif()
  endif()
endfunction()
...
function(xpExternPackage)
  ...
  if(XP_EXTERNPACKAGE_AUDIT_DEPS)
    ...
    if(DEFINED P_LIBRARIES)
      list(APPEND _audit_targets ${P_LIBRARIES})
    endif()
    if(DEFINED P_EXE)
      list(APPEND _audit_targets ${P_EXE})
    endif()
    if(_audit_targets)
      ipExternPackageAuditDeps(REPO_NAME "${P_REPO_NAME}" TARGETS ${_audit_targets} DEPS ${P_DEPS} PVT_DEPS ${P_PVT_DEPS})
    endif()
  endif()
