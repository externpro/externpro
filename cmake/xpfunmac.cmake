...
function(ipGetBoostCompiler result_var boost_path)
  # Get current system compiler using Boost-compatible format
  xpGetCompilerPrefix(system_compiler MATCH_BOOST)
  # Extract the compiler used to build Boost from the path
  cmake_path(GET boost_path FILENAME boost_dir_name)
  string(REGEX MATCH "boost-[^-]+-([^-]+)-[^-]+" boost_path_match ${boost_dir_name})
  if(CMAKE_MATCH_1)
    set(boost_version ${CMAKE_MATCH_1})
    # Convert boost compiler to Boost_COMPILER format (e.g., clang17 -> clang-darwin17)
    if(boost_version MATCHES "clang(.*)")
      if(${CMAKE_SYSTEM_NAME} STREQUAL Darwin)
        set(boost_compiler_string "clang-darwin${CMAKE_MATCH_1}")
      else()
        set(boost_compiler_string "clang${CMAKE_MATCH_1}")
      endif()
    else()
      set(boost_compiler_string ${boost_version})
    endif()
    # Compare system compiler with Boost's compiler
    if(NOT system_compiler STREQUAL boost_compiler_string)
      # Return the Boost compiler value
      set(${result_var} ${boost_compiler_string} PARENT_SCOPE)
    else()
      # Return empty string if compilers match
      set(${result_var} "" PARENT_SCOPE)
    endif()
  else()
    # Return empty string if we can't extract compiler from path
    set(${result_var} "" PARENT_SCOPE)
  endif()
endfunction()
...
function(ipDownload url sha dst)
  ...
  # Check if the destination file exists and verify its hash
  if(EXISTS "${dst}")
    file(SHA256 "${dst}" got_sha)
    if("${got_sha}" STREQUAL "${sha}")
      return()
    endif()
  endif()
  set(max_retries 8)
...
function(ipDownloadManifestFromRepo dst)
  ...
  # Check if manifest already exists locally and verify its hash
  if(EXISTS "${_dst}")
    file(SHA256 "${_dst}" got_sha)
    if("${got_sha}" STREQUAL "${_sha}")
...
function(xpFindPkg)
  ...
  foreach(req_pkg ${FP_PKGS})
    # Handle alias mapping - determine actual package to find
    if(DEFINED ap_${req_pkg})
      set(package ${ap_${req_pkg}})
    else()
      set(package ${req_pkg})
    endif()
    string(TOUPPER ${package} PKG)
    string(TOLOWER ${package} pkg)
    if(${package}_FOUND AND EXISTS ${${package}_DIR})
      continue()
    endif()
    if(DEFINED xp_${package})
      ...
      elseif(package STREQUAL "boost" AND EXISTS ${pth}/share/cmake/boost-config.cmake)
        ...
        ipGetBoostCompiler(boost_compiler_needed ${pth})
        if(boost_compiler_needed)
          set(Boost_COMPILER ${boost_compiler_needed})
        endif()
      ...
      # Package marker can override (unless global OFF)
      if(NOT FIND_PACKAGE_CMAKE_SCRIPT AND EXISTS ${pth}/share/cmake/findxpro.cmake)
        set(use_cmake_script TRUE)
      endif()
      if(use_cmake_script)
      ...
      if(DEFINED ${package}_CONFIG AND ${package}_CONFIG MATCHES "\\.cps$")
        ...
        if(NOT DEFINED ${PKG}_VER)
          set(${PKG}_VER ${${package}_VERSION})
          list(APPEND reqVars ${PKG}_VER)
        endif()
        if(NOT DEFINED ${req_pkg}_VERSION)
          set(${req_pkg}_VERSION ${${package}_VERSION})
          list(APPEND reqVars ${req_pkg}_VERSION)
        endif()
      ...
      if(DEFINED ${package}_FOUND)
        set(${req_pkg}_FOUND ${${package}_FOUND})
        set(${PKG}_FOUND ${${package}_FOUND})
        set(${pkg}_FOUND ${${package}_FOUND})
        list(APPEND reqVars ${package}_FOUND ${req_pkg}_FOUND ${PKG}_FOUND ${pkg}_FOUND)
      elseif(DEFINED ${PKG}_FOUND)
        set(${req_pkg}_FOUND ${${PKG}_FOUND})
        set(${package}_FOUND ${${PKG}_FOUND})
        set(${pkg}_FOUND ${${PKG}_FOUND})
        list(APPEND reqVars ${PKG}_FOUND ${req_pkg}_FOUND ${package}_FOUND ${pkg}_FOUND)
      elseif(DEFINED ${req_pkg}_FOUND)
        set(${package}_FOUND ${${req_pkg}_FOUND})
        set(${PKG}_FOUND ${${req_pkg}_FOUND})
        set(${pkg}_FOUND ${${req_pkg}_FOUND})
        list(APPEND reqVars ${req_pkg}_FOUND ${package}_FOUND ${PKG}_FOUND ${pkg}_FOUND)
      elseif(DEFINED ${pkg}_FOUND)
        set(${req_pkg}_FOUND ${${pkg}_FOUND})
        set(${package}_FOUND ${${pkg}_FOUND})
        set(${PKG}_FOUND ${${pkg}_FOUND})
      ...
      list(REMOVE_DUPLICATES reqVars)
      foreach(var ${reqVars})
...
function(xpProDeps)
  ...
  list(SORT pros)
  ipProDepsInit()
  if(NOT DEFINED __xp_downloading_manifest_message_shown)
    message(STATUS "Downloading manifest files for all externpro dependencies...")
    set(__xp_downloading_manifest_message_shown TRUE CACHE INTERNAL "Flag to track if downloading manifest message was shown")
  endif()
  foreach(pro ${pros})
...
function(ipExternPackageExtractDepNames _out items project_targets)
  ...
  foreach(_it IN LISTS items)
    if(_it MATCHES "^\\\$<")
      ...
      string(REGEX REPLACE "^\\\$<([^:]+):?([^>]*)>" "\\2" _name "${_it}")
    else()
      set(_name "${_it}")
    endif()
    if(TARGET "${_it}" OR TARGET "${_name}")
      # Check if dependency is internal
      set(_is_internal FALSE)
      if(_name IN_LIST project_targets)
      ...
      if(NOT _is_internal)
        # Convert target name to package name
        if(_name MATCHES "^([^:]+)::")
          set(_pkg_name "${CMAKE_MATCH_1}")
        else()
          set(_pkg_name "${_name}")
        endif()
        list(APPEND _names "${_pkg_name}")
      endif()
    endif()
  endforeach()
  if(_names)
...
function(ipExternPackageInferDeps _out_deps)
  ...
  foreach(_t IN LISTS P_TARGETS)
    ...
    if(_iface AND NOT _iface STREQUAL "_iface-NOTFOUND")
      list(APPEND _all_iface ${_iface})
    endif()
    get_target_property(_link ${_t} LINK_LIBRARIES)
    if(_link AND NOT _link STREQUAL "_link-NOTFOUND")
...
function(ipExternPackageComputeDiffs missing_var extra_var infer_list provided_list)
  # Initialize with the full lists
  set(_missing "${infer_list}")
  set(_extra "${provided_list}")
  # Only remove common items if both lists have content
  if((DEFINED infer_list AND NOT infer_list STREQUAL "") AND (DEFINED provided_list AND NOT provided_list STREQUAL ""))
    # Both lists have items - remove common items
    list(REMOVE_ITEM _missing ${provided_list})
    list(REMOVE_ITEM _extra ${infer_list})
  endif()
  set(${missing_var} "${_missing}" PARENT_SCOPE)
  set(${extra_var} "${_extra}" PARENT_SCOPE)
endfunction()
...
function(ipExternPackageAuditDeps provided_deps targets)
  ...
  # Always show comparison when audit is called
  message(STATUS " inferred='${_infer_deps}'")
  message(STATUS " provided='${provided_deps}'")
...
function(xpExternPackage)
  ...
  if(XP_EXTERNPACKAGE_AUDIT_DEPS)
    if(DEFINED P_LIBRARIES)
      message(STATUS "Audit deps LIBRARIES: ${P_LIBRARIES}")
      ipExternPackageAuditDeps("${P_DEPS}" "${P_LIBRARIES}")
    endif()
    if(DEFINED P_EXE)
      message(STATUS "Audit deps EXE: ${P_EXE}")
  ...
  if(DEFINED LIBS OR DEFINED EXT1 OR DEFINED EXT2 OR DEFINED EXT3 OR DEFINED EXT4 OR DEFINED ALIASES)
  ...
  ###############
  # findxpro.cmake marker for cmake script requirement
  if(P_FIND_XPRO_CMAKE)
    set(xpFindxproFile ${xproBinDir}/findxpro.cmake)
    file(WRITE ${xpFindxproFile}
      "# Generated by xpExternPackage()\n"
      "# Marker indicating this package requires find_package() to use cmake script\n"
      "# instead of CPS files for this platform/compiler combination\n"
      "# Platform: ${CMAKE_SYSTEM_NAME}-${CMAKE_SYSTEM_PROCESSOR}\n"
      "# Compiler: ${CMAKE_CXX_COMPILER_ID} ${CMAKE_CXX_COMPILER_VERSION}\n"
      )
    install(FILES ${xpFindxproFile} DESTINATION ${CMAKE_INSTALL_CMAKEDIR} ${XP_COMPONENT})
  endif()
  ###############
...
macro(xpCommonFlags)
  ...
  elseif((CMAKE_C_COMPILER_ID STREQUAL GNU) OR (CMAKE_CXX_COMPILER_ID STREQUAL GNU) OR (${CMAKE_C_COMPILER_ID} MATCHES "Clang") OR (${CMAKE_CXX_COMPILER_ID} MATCHES "Clang"))
    ...
    if(CMAKE_C_COMPILER AND CMAKE_C_COMPILER_ID)
      ...
      # Only add SSE flags on x86/x64 architectures
      if(CMAKE_SYSTEM_PROCESSOR MATCHES "x86_64|x86|AMD64|x64")
