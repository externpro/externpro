  ...
  elseif(NOT "${_len}" STREQUAL "0")
    ...
    foreach(i RANGE 0 ${_last})
      ...
      if(NOT "${_dep_err}" STREQUAL "" AND "${_dep_obj}" STREQUAL "")
        ...
        string(JSON _dep_name GET "${_dep_obj}" name)
        list(APPEND _deps_list "${_dep_name}")
        # Create package-specific dependency variable for version checking
        string(JSON _dep_repo GET "${_dep_obj}" repo)
        string(JSON _dep_tag GET "${_dep_obj}" tag)
        string(JSON _dep_sha GET "${_dep_obj}" manifest_sha256)
  ...
  if(DEFINED deps)
    foreach(dep ${deps})
      ...
      if(NOT DEFINED xp_${dep_lc})
        message(FATAL_ERROR "ipManifestDepsFromVarsJson: 'xp_${dep_lc}' is not defined")
      endif()
      # Parse the dependency information
      cmake_parse_arguments(DEP "" "REPO;TAG;MANIFEST_SHA256;MANIFEST_HASH" "" ${xp_${dep_lc}})
      # Create a JSON object with dependency version info
      set(_dep_json "{")
      string(APPEND _dep_json "\"name\": \"${dep_lc}\"")
      if(DEFINED DEP_REPO)
      ...
      string(JSON _deps_json SET "${_deps_json}" -1 "${_dep_json}")
    endforeach()
  endif()
