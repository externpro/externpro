...
function(ipDownloadExtract url sha pth)
  ...
  set(_pth ${XPRO_DIR}/xpx/${pkgbase}-xpro/share/cmake)
  if(NOT IS_DIRECTORY "${_pth}")
    set(_pth ${XPRO_DIR}/xpx/${pkgbase}/share/cmake)
  endif()
  if(NOT IS_DIRECTORY "${_pth}")
...
function(ipGetPkgFromManifest manifestFile pkg sha)
  ...
  if(_ext STREQUAL ".json")
    ipGetPkgFromManifestJson("${manifestFile}" ${pkg} ${sha})
  else()
    ipGetPkgFromManifestCMake("${manifestFile}" ${pkg} ${sha})
  endif()
endfunction()
...
function(xpFindPkg)
  ...
  foreach(package ${FP_PKGS})
    ...
    if(DEFINED xp_${pkg})
      ...
      unset(xpuse-${pkg}_DIR CACHE)
      find_package(xpuse-${pkg} BYPASS_PROVIDER REQUIRED PATHS ${pth} NO_DEFAULT_PATH)
      mark_as_advanced(xpuse-${pkg}_DIR)
    ...
    foreach(dep ${P_DEPS})
      if(DEFINED xp_${dep} AND DEFINED xp_${dep}_${P_PKG})
        ...
        if(NOT "${_pinned_key}" STREQUAL "${_parent_key}")
          ...
          cmake_parse_arguments(PIN "" "REPO;TAG;MANIFEST_SHA256" "" ${xp_${dep}})
          cmake_parse_arguments(PAR "" "REPO;TAG;MANIFEST_SHA256" "" ${xp_${dep}_${P_PKG}})
...
function(ipProDepsLoadManifest manifestFile)
  if(NOT EXISTS "${manifestFile}")
  ...
  if("${manifestFile}" MATCHES "\\.manifest\\.json$")
    file(READ "${manifestFile}" _json)
    string(JSON _base ERROR_VARIABLE _base_err GET "${_json}" base)
    if("${_base_err}" STREQUAL "" AND NOT "${_base}" STREQUAL "null")
      set(XP_MANIFEST_BASE "${_base}")
    endif()
    string(JSON _web ERROR_VARIABLE _web_err GET "${_json}" web)
    if("${_web_err}" STREQUAL "" AND NOT "${_web}" STREQUAL "null")
      set(XP_MANIFEST_WEB "${_web}")
    endif()
    string(JSON _up ERROR_VARIABLE _up_err GET "${_json}" upstream)
    if("${_up_err}" STREQUAL "" AND NOT "${_up}" STREQUAL "null")
      set(XP_MANIFEST_UPSTREAM "${_up}")
    endif()
    string(JSON _desc ERROR_VARIABLE _desc_err GET "${_json}" desc)
    if("${_desc_err}" STREQUAL "" AND NOT "${_desc}" STREQUAL "null")
      set(XP_MANIFEST_DESC "${_desc}")
    endif()
    string(JSON _lic ERROR_VARIABLE _lic_err GET "${_json}" license)
    if("${_lic_err}" STREQUAL "" AND NOT "${_lic}" STREQUAL "null")
      set(XP_MANIFEST_LICENSE "${_lic}")
    endif()
    string(JSON _xpd ERROR_VARIABLE _xpd_err GET "${_json}" xpdiff)
    if("${_xpd_err}" STREQUAL "" AND NOT "${_xpd}" STREQUAL "null")
      set(XP_MANIFEST_XPDIFF "${_xpd}")
    endif()
    set(_deps)
    string(JSON _deps_len ERROR_VARIABLE _deps_err LENGTH "${_json}" deps)
    if("${_deps_err}" STREQUAL "")
      if(_deps_len GREATER 0)
        math(EXPR _deps_last "${_deps_len} - 1")
        foreach(i RANGE 0 ${_deps_last})
          string(JSON _d ERROR_VARIABLE _d_err GET "${_json}" deps ${i})
          if("${_d_err}" STREQUAL "" AND NOT "${_d}" STREQUAL "null")
            list(APPEND _deps "${_d}")
          endif()
        endforeach()
      endif()
      if(_deps)
        list(TRANSFORM _deps TOLOWER)
        set(XP_MANIFEST_DEPS ${_deps})
      endif()
    endif()
    set(_pvt_deps)
    string(JSON _pvt_len ERROR_VARIABLE _pvt_err LENGTH "${_json}" pvt_deps)
    if("${_pvt_err}" STREQUAL "")
      if(_pvt_len GREATER 0)
        math(EXPR _pvt_last "${_pvt_len} - 1")
        foreach(i RANGE 0 ${_pvt_last})
          string(JSON _d ERROR_VARIABLE _d_err GET "${_json}" pvt_deps ${i})
          if("${_d_err}" STREQUAL "" AND NOT "${_d}" STREQUAL "null")
            list(APPEND _pvt_deps "${_d}")
          endif()
        endforeach()
      endif()
      if(_pvt_deps)
        list(TRANSFORM _pvt_deps TOLOWER)
        set(XP_MANIFEST_PVT_DEPS ${_pvt_deps})
      endif()
    endif()
  else()
    include("${manifestFile}")
...
function(ipProDepsRow)
  ...
  block(PROPAGATE deps dot rme depsCheckCount depsMismatchCount depsMismatchReport)
    ...
    if((DEFINED P_MANIFEST_SHA256 OR DEFINED P_MANIFEST_HASH) AND DEFINED P_REPO AND DEFINED P_TAG)
      ipDownloadManifestFromRepo(dst REPO ${P_REPO} TAG ${P_TAG} MANIFEST_SHA256 ${P_MANIFEST_SHA256} MANIFEST_HASH ${P_MANIFEST_HASH})
      ipProDepsLoadManifest(${dst})
    endif()
    if(DEFINED P_MANIFEST_FILE AND EXISTS ${P_MANIFEST_FILE})
      ipProDepsLoadManifest(${P_MANIFEST_FILE})
    endif()
    # track dependencies
...
function(ipManifestDepsFromVars _out deps)
  foreach(dep ${deps})
    ...
    if(NOT DEFINED xp_${dep_lc})
      message(FATAL_ERROR "ipManifestDepsFromVars: 'xp_${dep_lc}' is not defined")
    endif()
    string(REPLACE ";" " " xp_${dep_lc} "${xp_${dep_lc}}")
    string(REPLACE " MANIFEST" "\n  MANIFEST" xp_${dep_lc} "${xp_${dep_lc}}")
    set(out "${out}set(xp_${dep_lc}_${lcRepoName} ${xp_${dep_lc}}\n  )\n")
  endforeach()
