...
function(ipGetPkgFromManifest manifestFile pkg sha)
  ...
  elseif(CMAKE_SYSTEM_PROCESSOR MATCHES "^(x86_64|amd64|AMD64)$")
    ...
    message(FATAL_ERROR "ipGetPkgFromManifest: unhandled CMAKE_SYSTEM_PROCESSOR='${CMAKE_SYSTEM_PROCESSOR}'")
  endif()
  if(NOT EXISTS "${manifestFile}")
    message(FATAL_ERROR "ipGetPkgFromManifest: manifest file not found: '${manifestFile}'")
  endif()
  unset(XP_MANIFEST_ARTIFACTS)
  include("${manifestFile}")
  if(NOT DEFINED XP_MANIFEST_ARTIFACTS)
    message(FATAL_ERROR "ipGetPkgFromManifest: manifest missing XP_MANIFEST_ARTIFACTS: '${manifestFile}'")
  endif()
  ...
  if(found_pkg STREQUAL "")
    message(FATAL_ERROR "ipGetPkgFromManifest: no artifact matched '${pfx}-${platform}' (or fallback '${platform}') in '${manifestFile}'")
  endif()
  ipNormalizeManifestVarName(norm "${found_pkg}")
  set(shaVar "XP_ARTIFACT_SHA256__${norm}")
  if(NOT DEFINED ${shaVar})
    message(FATAL_ERROR "ipGetPkgFromManifest: missing sha variable '${shaVar}' for artifact '${found_pkg}' in '${manifestFile}'")
  endif()
  string(LENGTH "${${shaVar}}" sha_len)
  if(NOT sha_len EQUAL 64)
    message(FATAL_ERROR "ipGetPkgFromManifest: invalid SHA length (${sha_len}) for '${found_pkg}' in '${manifestFile}'")
  endif()
...
function(ipGetPrefixPath pfx pth)
  ...
  elseif(DEFINED P_REPO AND DEFINED P_TAG AND DEFINED P_SHA256_${SYS_NAME})
    ...
    set(sha ${P_SHA256_${SYS_NAME}})
  elseif(DEFINED P_MANIFEST_FILE AND EXISTS ${P_MANIFEST_FILE})
    ipGetPkgFromManifest(${P_MANIFEST_FILE} pkg sha) # set pkg and sha
    # assume MANIFEST_FILE and pkg are in the same directory
    cmake_path(GET P_MANIFEST_FILE PARENT_PATH pkg_dir)
    set(url file://${pkg_dir}/${pkg})
  elseif(DEFINED P_PKG AND DEFINED P_REPO AND DEFINED P_TAG AND DEFINED P_MANIFEST_SHA256)
    string(REGEX REPLACE ".*/" "" PKG_NAME ${P_REPO})
    set(dst ${xprodir}/xpd/manifests/${PKG_NAME}-${P_TAG}.manifest.cmake)
